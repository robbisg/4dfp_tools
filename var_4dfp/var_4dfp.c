/*$Header: /data/petsun4/data1/src_solaris/var_4dfp/RCS/var_4dfp.c,v 1.22 2013/07/18 04:35:06 avi Exp $*/
/*$Log: var_4dfp.c,v $
 * Revision 1.22  2013/07/18  04:35:06  avi
 * isnormal prototype in sunmath
 *
 * Revision 1.21  2013/07/18  04:25:37  avi
 * more complete usage; report number of underined voxels; isnan logic -> isnormal logic
 *
...
#include <stdio.h>
#include <string.h>
#include <math.h>		/* defines isnormal() in Linux */
#include <float.h>
#ifndef _FEATURES_H
...
/* global variables */
/********************/
static char rcsid[] = "$Id: var_4dfp.c,v 1.22 2013/07/18 04:35:06 avi Exp $";
...
/*******************/
	...
	int		imgdim[4], dimension, nf_total, nundefined, nf_func, nf_anat = 0;
...
/************************/
	...
	if (k < 1) {
		...
		printf ("N.B.:\toptions {-v -s -m -z} are mutually exclusive\n");
		printf ("N.B.:\tabsent -G voxelwise mean is individually computed over each run in conc\n");
...
/********************************/
	for (iframe = kk = ifile = 0; ifile < nfile; ifile++) {
		...
		for (k = j = 0; j < ((conc_flag) ? conc_block.nvol[ifile] : imgdim[3]); j++) {
			...
			for (i = 0; i < dimension; i++) {
				if (imgt[i] == (float) 1.e-37 || !isnormal (q = imgt[i])) imgs[i] = 0.;
				imgu[ifile][i]	+= imgt[i];
				imga[i]		+= imgt[i];
			}
		}
	...
	assert (kk == nf_func);
	for (nundefined = i = 0; i < dimension; i++) {
		imga[i] /= kk;
		if (!imgs[i]) nundefined++;
	}
	printf ("undefined voxel count=%d\n", nundefined);
