/*$Header: /data/petsun4/data1/src_solaris/qnt_4dfp/RCS/qnt_4dfp.c,v 1.22 2014/08/23 05:40:39 avi Exp $*/
/*$Log: qnt_4dfp.c,v $
 * Revision 1.22  2014/08/23  05:40:39  avi
 * more generous string and filename buffers (MAXL -> 4*MAXL)
 *
 * Revision 1.21  2014/03/04  06:50:46  avi
 * eliminate hard-coded format buffer initial size limit
 *
 * Revision 1.20  2013/05/30  01:21:13  avi
 * output format modified to insert spaces between fields
 *
...
#endif
#include <endianio.h>
#include <conc.h>		/* #define MAXL 256 */
...
/********************/
char	program[MAXL];
static char rcsid[]= "$Id: qnt_4dfp.c,v 1.22 2014/08/23 05:40:39 avi Exp $";
...
/*************/
	...
	FILE            *imgfp, *mskfp;
	char            imgroot[4*MAXL], imgfile[4*MAXL], mskroot[4*MAXL], mskfile[4*MAXL];
...
/************************/
	for (j = 0, i = 1; i < argc; i++) {
		if (*argv[i] == '-') {
			strncpy (command, argv[i], 1023); ptr = command;
...
/********************************************/
	if (conc_flag) {
		...
		strncpy (imgfile, conc_block.lstfile, 4*MAXL - 1);
