...
pushd algebra_4dfp
make -f generate_rand_4dfp.mak		release
if ($status) exit $status
make -f imgopr_4dfp.mak			release
if ($status) exit $status
make -f ROI_resolve_4dfp.mak		release
if ($status) exit $status
make -f ROI_resolve_multivol_4dfp.mak	release
if ($status) exit $status
...
if ($status) exit $status
make -f set_undefined_4dfp.mak		release
if ($status) exit $status
popd
